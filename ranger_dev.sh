...
then
    ALL_SERVICES="${ALL_SERVICES},ranger-debug,ranger-usersync-debug"
fi
...
#   cp -v target/ranger-*-usersync.tar.gz dev-support/ranger-docker/dist/
#   cp -v target/ranger-*-tagsync.tar.gz dev-support/ranger-docker/dist/
#   cp -v target/ranger-*-kms.tar.gz dev-support/ranger-docker/dist/
...
cd ${RD_HOME}
# DOn't know hy, in my machine I have docker-compose v1 and docker compose v2
echo "docker compose ${COMPOSE_FILES} ${DOCKER_ACTION}"
